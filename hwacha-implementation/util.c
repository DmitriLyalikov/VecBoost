...
/*
* Hwacha vector implementation to convert feature depth to channel, height, width format
* Execution in cycles = 5 + Channels * (Height * 2) + (8 * w / MAXVL)
*/
void convert_fd_to_nchw(float* in, int w, int h, int c, float *out)
{
    setvcfg(0, 1, 0, 1); // config for one word vector register and one pred
